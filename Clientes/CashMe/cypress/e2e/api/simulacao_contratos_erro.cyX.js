describe('CashMe | Simulação de Evolução de Contrato', () => {
  ...
  function validarGridEvolucaoTeorica(grid) {
    ...
    if (Array.isArray(grid) && grid.length === 13) {
      ...
      cy.softAssert(primeira.PRESTACAO === "73.98", `PRESTACAO da 1ª parcela esperado '73.98', veio '${primeira.PRESTACAO}'`);
      cy.softAssert(ultima.PREPRZ === "012/012", `PREPRZ da última parcela esperado '012/012', veio '${ultima.PREPRZ}'`);
      cy.softAssert(ultima.SALDODEV === "0.01", `SALDODEV da última parcela esperado '0.01', veio '${ultima.SALDODEV}'`);
    }
  }
  ...
  it('', () => {
    cy.request({
      ...
      cy.softAssert(body.VA_IOF === '2107.61', `Valor de VA_IOF esperado '2107.61', veio '${body.VA_IOF}'`);
    }).then(() => {
      cy.checkSoftAsserts();
    });
  });
